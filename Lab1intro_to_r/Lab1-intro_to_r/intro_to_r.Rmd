...
The years that are included in this data are the years between 1940 until 2002. We can examine the data set below and confirm the above.
...
Thus, for our data set we have 63 rows and 3 columns.
...
Both data sets contain information about boys and girls between a certain time range, however they refer to different years. Also the scale of the present is larger that the Arbuthnot's data set.

### Make a plot that displays the boy-to-girl ratio for every year in the data set. What do you see? Does Arbuthnot's observation about boys being born in greater proportion than girls hold up in the U.S.? Include the plot in your response.

```{r calculate-boy-to-girl-ratio}
boy_girl_ratio = present$boys /  present$girls
```

```{r plot-boy-to-girl-ratio-for-every-year}
plot(present$year, boy_girl_ratio, type = "l")

```

We can observe that the number of boys that are born in the U.S is continuously decreasing from 1940 to 2002. Even, if the ratio at some point increases, the overall conclusion is that the ratio is dropping and the number of born boys is becoming nearly equal to the girls.

### In what year did we see the most total number of births in the U.S.?

```{r calculate the max and get the year}
births = present$boys + present$girls
max_idx = which.max(births)
cat('Max births: ')
cat(max(births))
cat("\n")
cat('Year: ')
cat(present$year[max_idx])
```
